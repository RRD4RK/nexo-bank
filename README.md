...
3.  Compile a amostra Sass para demonstrar variáveis/mixins: `npm run build:css`.
...
- **ID 02:** Grid e utilitários Bootstrap nas páginas internas.
- **ID 03:** `.dashboard-layout`, `.operation-menu`, `.hero-home` e media queries no CSS.
- **ID 07:** variáveis em `_variable.scss`, mixins em `_mixins.scss` e uso em `main.scss`.
- **ID 10:** `<picture>`, WebP, `srcset` e fallback PNG no logo da home/navbar.
